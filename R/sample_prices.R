...
# Title:    Sampling of price data
# Author:   Sebastian Weinand
# Date:     2023-07-05

# random logarithmic pi-parameter; non-visible in package:
...
# random delta-parameter; non-visible in package:
...
# sample minimum connected data; non-visible in package:
...
# sample gaps in price data while keeping connectedness:
gaps <- function(r, n, amount=0, prob=NULL, pairs=FALSE, exclude=NULL){
  ...
  .check.char(x=r)
  .check.char(x=n)
  .check.num(x=amount, miss.ok=TRUE, null.ok=FALSE, int=c(-.Machine$double.eps, 1))
...
prices <- function(R, N, weights=NULL, gaps=0, settings=list()){
  ...
  # gaps        see gaps()
  # settings    list of
  #             o gaps.prob: see argument 'prob' in gaps() 
  #             o gaps.pairs: see argument 'pairs' in gaps()
  #             o gaps.exclude: see argument 'exclude' in gaps()
  ...
  # check input for "R":
  if(missing(R)){stop("'R' is missing.")}
  if(!is.numeric(R)){stop("'R' must be numeric.")}
  if(length(R) != 1){stop("'R' must contain at least one element.")}
  if(is.na(R)){stop("'R' is not allowed to contain NAs.")}
  
  # check input for "N":
  if(missing(N)){stop("'N' is missing.")}
  if(!is.numeric(N)){stop("'N' must be numeric.")}
  if(length(N) != 1){stop("'N' must contain at least one element.")}
  if(is.na(N)){stop("'N' is not allowed to contain NAs.")}
  
  # set to default values if missing:
  if(missing(weights)) weights <- NULL
  if(missing(gaps)) gaps <- NULL
  ...
  r <- paste0("r", formatC(x=1:R, width=log10(R)+1, format="d", flag="0"))
  n <- paste0("n", formatC(x=1:N, width=log10(N)+1, format="d", flag="0"))
  ...
  if(is.null(weights)){
    ...
    dt[, "weight" := weights(r=region, n=product, type=weights)]
  }
  
  # true regional price spreads across products:
  rhs.vars <- all.vars(weights)
  if(length(rhs.vars) > 1L){
    # w <- dt$weight
    delta <- rep(1, N)
  }else{
    w <- unique(x=dt, by="product")$w
  ...
  # introduce gaps:
  if(!is.null(gaps)){
    dt <- dt[!gaps(r=region, n=product, amount=gaps, prob=settings$gaps.prob, pairs=settings$gaps.pairs, exclude=settings$gaps.exclude), ]
  }
  
  # set column order and key:
  setcolorder(x=dt, c("region", "product", "weight", "price"))
  setkeyv(x=dt, cols=c("product", "region"))
  dt[, "error" := NULL]
  if(is.null(weights)) dt[, "weight" := NULL]
