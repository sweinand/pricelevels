...
* and the *individual product* $(n=1,\ldots,N_b)$, for which prices $p_{bi}^r$ are recorded.

For transaction and supermarket scanner data, purchased quantities $q_{bi}^r$ of the individual products are often available.
...
**Connected price data.** Suppose we have prices for 5 products (all belonging to the same basic heading) in 4 regions. These data are stored in the data.table `dt`. Transforming `dt` into a price matrix (rows: products; columns: regions) shows that the data exhibit gaps as products `4` and `5` are not priced in regions `c` and `d`, respectively.
```{r}
...
**Non-connected price data.** Let's assume now that prices for products `1`, `2`, and `3` are no longer available in regions `a` and `b` (for example because these products are not sold there anymore). This situation is captured in data.table `dt2`. In total, the price data still include 4 regions and 5 products. However, the price matrix reveals that the pattern of gaps changed. 
```{r}
...
\begin{equation}
  p_{bi}^r = \pi_{bi} \ (P^r)^{\delta_{bi}} \ u_i^r \ ,
\end{equation}
where the price of product $i$ in region $r$, $p_{bi}^r$, is explained by the general product price, $\pi_{bi}$, the product-specific elasticities $\delta_{bi}$, and the regional price level, $P^r$. The error term $u_{bi}^r$ follows a lognormal distribution.
...
The following code snippet shows how easily a connected price dataset with gaps, sales and expenditure weights can be simulated.
...
boxplot(log(price)~paste(group, product, sep=":"), data=srp$data)
...
A price comparison of region `2` to `1` should give the same result as the reciprocal of the price comparison of region `1` to `2`. This requirement is denoted as the *country reversal test*. However, as shown below, only the Jevons index and the Dutot index comply with this requirement for the considered dataset.
```{r}
...
Multilateral price indices simultaneously use the prices of all regions involved in the price comparison to compute a set of transitive price index numbers - irrespective if there are data gaps. The `spin`-package offers the three most prominent multilateral price indices, and a newly developed extension:
...
The CPD and NLCPD methods are implemented in functions `cpd()` and `nlcpd()`. The functions can be used similarly to those for bilateral indices as shown below. However, if wanted by the user, they provide some additional information.
```{r}
...
```
The NLCPD method solves a nonlinear optimization problem. Therefore, it is computationally much slower than most other price indices. However, computation speed can be improved by using the Jacobian matrix instead of deriving it analytically during optimization. This can be achieved by changing the `settings` in `nlcpd()`.
```{r}
set.seed(123)
...
# estimate NLCPD model using different base regions:
...
Again, the following checks show that the price index numbers derived with the GEKS-Törnvist index are transitive:
```{r}
...
P1.geks <- dt.gaps[, geks(p=price, r=region, n=product, q=quantity, base="1",
                          settings=list(type="toernq", method="obs"))]
...
The Geary-Khamis method traces back to the work of @Geary1958 and @Khamis1972. It derives international average price, $\pi_n \ (n=1,\ldots,N)$, and regional price levels, $P^r \ (r=1,\ldots,R)$, from the following system of interrelated equations:

$$\pi_n = \frac{\sum_{r=1}^{R} p_n^r q_n^r / P^r}{\sum_{r=1}^{R} q_n^r} \quad \text{and} \quad P^r = \frac{\sum_{n=1}^{N} p_n^r q_n^r}{\sum_{n=1}^{N} \pi_n q_n^r} \ . $$
The formulas show that the Geary-Khamis index requires quantities. The system of equations can be solved iteratively or - as shown by @Diewert1999 - analytically using matrix algebra. Which approach is computationally faster depends on the data. 
```{r}
...
system.time(m1 <- dt.big[, gk(p=price, r=region, n=product, q=quantity,
                              settings=list(method="iterative"), simplify=FALSE)])

# matrix algebra:
system.time(m2 <- dt.big[, gk(p=price, r=region, n=product, q=quantity,
                              settings=list(method="solve"), simplify=FALSE)])

# compare results and computation time:
all.equal(m1, m2, tol=1e-05)
```

Also the Geary-Khamis price index is transitive.
```{r}
# estimate Geary-Khamis using different base regions:
