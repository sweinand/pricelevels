...
- Harmonic mean index: $$P_{\text{Harmonic}}^{sr} = \frac{N}{\sum_{n=1}^{N} p_n^s / p_n^r}$$
...
- Laspeyres index [@Laspeyres1981]: $$P_{\text{Laspeyres}}^{sr} = \frac{\sum_{n=1}^{N} p_n^r q_n^s}{\sum_{n=1}^{N} p_n^s q_n^s} = \sum_{n=1}^{N} w_n^s \left( p_n^r / p_n^s \right)$$
...
- Palgrave index: $$P_{\text{Palgrave}}^{sr} = \sum_{n=1}^N w_n^r \left( p_n^r / p_n^s \right)$$
- Marshall-Edgeworth index [@Marshall1887; @Edgeworth1925]: $$P_{\text{ME}}^{sr} = \frac{\sum_{n=1}^N p_n^r \left( q_n^r + q_n^s \right)}{\sum_{n=1}^N p_n^s \left( q_n^r + q_n^s \right)}$$
- Walsh index: $$P_{\text{Walsh}}^{sr} = \frac{\sum_{n=1}^{N} \sqrt{w_n^r w_n^s} \sqrt{p_n^r / p_n^s}}{\sum_{n=1}^{N} \sqrt{w_n^r w_n^s} \sqrt{p_n^s / p_n^r}}$$
- Törnqvist index [@Toernqvist1936]: $$P_{\text{Törnqvist}}^{sr} = \prod_{n=1}^N \left( p_n^r / p_n^s \right)^{\bar{w}_n^{sr} / \sum_{j=1}^N \bar{w}_j^{sr}} \quad , \text{with} \ \bar{w}_n^{sr} = \frac{1}{2} \left( w_n^r + w_n^s \right)$$
- Sato-Vartia index [@Sato1976; @Vartia1976]:  $$P_{\text{SV}}^{sr} = \prod_{n=1}^N \left( p_n^r / p_n^s \right)^{\bar{w}_n^{sr} / \sum_{j=1}^N \bar{w}_j^{sr}} \quad , \text{with} \ \bar{w}_n^{sr} = \begin{cases} \frac{w_n^r-w_n^s}{\ln{w_n^r}-\ln{w_n^s}} & w_n^r \neq w_n^s \\ w_n^s & w_n^r = w_n^s \end{cases}$$
- Theil index: $$P_{\text{Theil}}^{sr} = \prod_{n=1}^N \left( p_n^r / p_n^s \right)^{\bar{w}_n^{sr} / \sum_{j=1}^N \bar{w}_j^{sr}} \quad , \text{with} \ \bar{w}_n^{sr} = \left[ \frac{p_n^r q_n^r + p_n^s q_n^s}{2} \ p_n^r q_n^r \ p_n^s q_n^s \right]^{1/3} \ ,$$
